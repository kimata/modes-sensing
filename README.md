...
### 深夜に「Mode S データを N 秒間受信していません」が出る

深夜は受信圏内に航空機が居らず、30 秒以上データが来ないことが常態です。
接続自体は成功しているため、collector はこれを異常とは扱わず接続を張り直しながら待ち続けます
（10 分ごとに WARNING、受信再開時に INFO を出力）。
一方、接続エラーやデータを 1 行も受信せずに切断される状態が 10 回連続した場合は、
Slack 通知のうえプロセスを終了し、コンテナの再起動に委ねます。
