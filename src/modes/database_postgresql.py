...
def fetch_by_time_aggregated(  # noqa: PLR0913
    conn,
    time_start,
    time_end,
    distance,
    ...
    """
    指定された時間範囲と距離でデータを時間・高度ビンで集計して取得する
    ...
    Args:
        ...
        time_end: 終了時刻
        distance: 距離フィルタ
        time_bin_minutes: 時間ビンのサイズ（分）
        altitude_bin_meters: 高度ビンのサイズ（メートル）
        columns: 取得するカラムのリスト（風データを含む場合は wind_x, wind_y 等を指定）
        max_altitude: 最大高度フィルタ（Noneの場合はフィルタなし）

    Returns:
        集計されたデータのリスト（各レコードは time, altitude, temperature, その他の平均値を含む）

    """
    # 風データが必要かどうかを判定
    include_wind = columns is not None and any(
        col in columns for col in ["wind_x", "wind_y", "wind_speed", "wind_angle"]
    )

    # 基本の集計カラム
    time_bin_expr = f"""date_trunc('hour', time) +
        INTERVAL '1 minute' * (FLOOR(EXTRACT(MINUTE FROM time) / {time_bin_minutes}) * {time_bin_minutes})"""
    alt_bin_expr = f"FLOOR(altitude / {altitude_bin_meters})"
    alt_center = f"{alt_bin_expr} * {altitude_bin_meters} + {altitude_bin_meters}/2.0"

    select_columns = [
        f"{time_bin_expr} AS time",
        f"{alt_center} AS altitude",
    ...
    if include_wind:
        select_columns.extend(
            [
                ...
                # wind_angleは平均を取ると不正確になるため、wind_x, wind_yから再計算する
            ]
        )
    ...
    with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
        ...
        logging.info(
            "Elapsed time: %.2f sec (aggregated, time_bin=%d min, alt_bin=%d m, %s rows)",
            time.perf_counter() - start,
            time_bin_minutes,
            altitude_bin_meters,
            f"{len(data):,}",
