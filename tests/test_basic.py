...
#         raise FileNotFoundError(f"packet.dump file not found: {dump_file_path}")  # : TRY003, EM102

#     with dump_path.open("rb") as f:
#         return pickle.load(f)  # : S301
...
#                 def socket(self, socket_type):  # : ARG002
...
#     def test_packet_handler(handle, header, payload):  # : ARG001
#         """テスト用のパケットハンドラ"""
...
#             b"\xfe",  # 1バイト（不完全なヘッダー）
#         ]
#         return random.choice(dummy_types)  # : S311

#     def read(self, size):
#         """シリアルからのデータ読み取りをシミュレート"""
#         # ダミーデータの注入（20%の確率）
#         if self.inject_dummy_data and random.random() < 0.2 and size == 2:  # : S311
...
#         sharp_hems.sniffer.process_packet(handle, header, payload, lambda data: sense_dat
#         processed_count += 1
...
#     logging.info("Successfully processed %d serial packets with dummy data injection", l
...
# def test_server_all_packets(server_port):  # : C901
