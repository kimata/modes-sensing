...
- 高度-温度相関による外れ値検出（線形回帰 + 高度近傍統計、sklearn）
- PostgreSQL へのデータ保存
- 10種類のグラフ生成（散布図、ヒートマップ、等高線、温度、風向、鉛直プロファイルなど）
...
`receiver.py` の `_worker` は接続が終わるたびに再接続し、終わり方を `_ConnectionOutcome` で区別する：

| 終わり方              | 意味                                       | 扱い                                               |
| --------------------- | ------------------------------------------ | -------------------------------------------------- |
| `RECEIVED`            | 1 行以上受信した                           | 失敗カウントをリセットして即再接続                 |
| `SILENT_TIMEOUT`      | 1 行も受信せずソケットタイムアウト（無音） | 正常扱い（深夜は常態）。即再接続、ログはスロットル |
| `CLOSED_WITHOUT_DATA` | 1 行も受信せず切断／接続エラー             | 失敗カウント + 指数バックオフ                      |

連続失敗が `MODES_RECEIVER_MAX_RETRIES` を超えた場合はワーカーだけを止めず、
自プロセスへ SIGTERM を送り `collect.py` が exit 1 で終了してコンテナ再起動に委ねる
（ワーカーだけが止まると、liveness probe が失敗するまで Mode S データが欠測し続けるため）。
...
| 気温プロファイル | temperature_profile | 気温の鉛直プロファイル（末尾3時間） |
| ホドグラフ       | hodograph           | 風のホドグラフ（末尾3時間）         |
...
- **ミラーリポジトリ**: GitHub (`github.com/kimata/modes-sensing`)
